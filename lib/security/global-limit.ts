...
export const GLOBAL_TIERS: readonly GlobalLimitTier[] = [
   ...
   * (prefetches are excluded before we get here — see `shouldSkip`), so this is roughly a
   * very fast human browsing hard, and nowhere near a flood.
   */
...
export function tierFor(pathname: string): GlobalLimitTier {
  const [auth, bill, fallback] = GLOBAL_TIERS as [
    GlobalLimitTier,
    GlobalLimitTier,
    GlobalLimitTier,
  ];
  ...
  if (BILL_PREFIXES.some((p) => pathname.startsWith(p))) return bill;
  return fallback;
}

/**
 * Requests that must not be counted.
 *
 * **Prefetches.** `next/link` fires one per link in the viewport, so a catalogue page can
 * generate dozens of proxy-visible requests per navigation. Counting them would exhaust a
 * limit tuned for humans and lock out ordinary browsing — the failure would look like the
 * site randomly breaking while scrolling. Next's own CSP guide recommends excluding them
 * from proxy work for the same reason.
 *
 * **`/api/health`.** §9.4 puts uptime checks on it. A monitor that receives a 429 reports a
 * false outage, and an alert nobody can trust is worse than no alert. The endpoint runs one
 * trivial query and discloses nothing.
 */
export function shouldSkip(request: Request, pathname: string): boolean {
...
export async function consumeGlobalLimit(
  ...
  const key = `rl:global:${tier.name}:${ip}`;
